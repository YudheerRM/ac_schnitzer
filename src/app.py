...
BATCH_ZIP_NAME = "woocommerce_batch_export.zip"
...
def cleanup_batch_files():
    ...
    zip_path = OUTPUT_DIR / BATCH_ZIP_NAME
    if zip_path.exists():
        zip_path.unlink()
...
def main():
    ...
    use_formula = st.checkbox("Enable Pricing Formula", value=False, disabled=is_running)
    price_formula = st.text_input(
        "Pricing Formula (use 'x' for price)", 
        value="x * 1.2", 
        disabled=not use_formula or is_running,
        help="Example: x * 1.2 (adds 20%), x + 10 (adds 10 currency units)"
    )
    ...
    with col1:
        ...
        if st.session_state.get("batch_running", False):
            ...
            if return_code == 0:
                ...
                if create_batch_zip():
                    st.session_state.logs.append(f"ZIP archive created: {BATCH_ZIP_NAME}")
                    log_container.code("\n".join(st.session_state.logs), language="bash")
    ...
    with col2:
        ...
        display_names = {
            ...
            BATCH_ZIP_NAME: "Batch Export (ZIP)"
        }
